...
export default function CobroForm({ clienteId, cobro, onCerrar, actualizar }) {
  const edicion = !!cobro;
      ...
      if (edicion) {
        return {
          ...
          cobros: d.cobros.map((c) =>
            c.id === cobro.id ? { ...c, fechaPago, concepto, importe: imp, metodo, estado, nota } : c
          ),
        };
      }
      return {
        ...d,
        cobros: [...d.cobros, nuevoCobro({ clienteId, fechaPago, concepto, importe: imp, metodo, estado, nota })],
      };
    });
    onCerrar();
  };
